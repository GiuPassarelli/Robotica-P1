...
#o valor do atraso pode ser um problema se a rede estiver sobrecarregada (aumentar)
#atraso = 0.15E9 # 0.15 segundos. Em nanossegundos  #quando a rede estiver vazia

atraso = 0.6E9 # 0.15 segundos. Em nanossegundos
...
def roda_todo_frame(imagem):
	...
	try:
		...
		media, centro, area =  seguemodule_soldadinho.identifica_frame(cv_image)
		depois = time.clock()
...
if __name__=="__main__":
	...
	try:
		...
		while not rospy.is_shutdown():
			...
			if len(media) == 2 and media[0] == -1: # Nao viu a madfox
				vel = Twist(Vector3(0,0,0), Vector3(0,0,0))
			else:
